...
def param_writer(name, shape):
    ...
    # TODO: lifetime specifiers?
    print '\tfn write_params<\'a>(request: &mut SignedRequest<\'a>, obj: &\'a ' + name + ', location: Option<&ArgumentLocation>, location_name: &str) {'
...
def request_method(operation):
    ...
    if not ('input' in operation):
        ...
        print '\t\t' + input_name + 'Writer::write_params(&mut request, &input, None, &"".to_string());'
